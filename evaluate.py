...
class Evaluator:
    ...
    def _eval_hits(self, y_pred_pos, y_pred_neg, type_info):
        ...
        if type_info == 'torch':   # 137 mil edges neg ;  530 mil positivas
            kth_score_in_negative_edges = torch.topk(y_pred_neg, self.K)[0][-1] # edge neg # 100 das mais altas, todos os valores y_pred para essa edge_neg
            # médias dos scores y_pred de todas as edges positivas 
            hitsK = float(torch.sum(y_pred_pos > kth_score_in_negative_edges).cpu()) / len(y_pred_pos)
                #     y_pred_pos, y_pred_neg, type_info = self._parse_and_check_input(input_dict)
            if self.K == 100:
                #roc_auc = self._eval_roc_auc(y_pred_pos, y_pred_neg, type_info)
                acc = self._eval_accuracy(y_pred_pos, y_pred_neg, type_info)
                f1_score = self._eval_f1score(y_pred_pos, y_pred_neg, type_info)
                return {'hits@{}'.format(self.K): hitsK, 'acc': acc['acc'], 'f1_score': f1_score['f1score']}
            else:
    ...
    def _eval_accuracy(self,y_pred_pos,y_pred_neg, type_info):
        if type_info == 'torch':
            ones = np.ones(len(y_pred_pos))
            zeros = np.zeros(len(y_pred_neg))
            y_true = np.append(ones,zeros)
            dict1={}
            z = np.append(y_pred_pos.numpy(), y_pred_neg.numpy())
            dict1["acc"] = (z > 0.5).astype(np.int_)
            acc = sklearn.metrics.accuracy_score(y_true,dict1["acc"])
        else:
    ...
    def _eval_f1score(self,y_pred_pos,y_pred_neg, type_info):
        if type_info == 'torch':
            ones = np.ones(len(y_pred_pos))
            zeros = np.zeros(len(y_pred_neg))
            y_true = np.append(ones,zeros)
            dict1={}
            y = np.append(y_pred_pos.numpy(), y_pred_neg.numpy())
            dict1["f1score"] = (y > 0.5).astype(np.int_)
            f1score = sklearn.metrics.f1_score(y_true,dict1["f1score"])
        else:
        # if type_info is numpy
            ones = np.ones(len(y_pred_pos))
            zeros = np.zeros(len(y_pred_neg))
