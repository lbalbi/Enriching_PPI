...
class Logger(object):
    ...
    def add_result(self, run, result):
        assert len(result) == 3 or len(result) == 5
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            print(f"  Final Test Hits: {result[argmax, 2]:.2f}")
            if len(result[0]) == 5:
                print(f"  Final Test Acc: {result[argmax, 3]:.2f}")
                print(f"  Final Test F1: {result[argmax, 4]:.2f}")
            ...
            for r in result:
                ...
                if len(r[0]) == 5:
                    ...
                    best_results.append((train1, valid, train2, test, test_acc, test_f1))
                else: best_results.append((train1, valid, train2, test))

            best_result = torch.tensor(best_results)

            print(f"All runs:")
            r = best_result[:, 0]
            print(f"Highest Train Hits: {r.mean():.2f} ± {r.std():.2f}")
            r = best_result[:, 1]
            print(f"Highest Valid Hits: {r.mean():.2f} ± {r.std():.2f}")
            r = best_result[:, 2]
            print(f"  Final Train Hits: {r.mean():.2f} ± {r.std():.2f}")
            r = best_result[:, 3]
            print(f"   Final Test Hits: {r.mean():.2f} ± {r.std():.2f}")
            if len(best_result[0]) == 6:
                r = best_result[:, 4]
                print(f"   Final Test Acc: {r.mean():.2f} ± {r.std():.2f}")
                r = best_result[:, 5]
                print(f"   Final Test F1: {r.mean():.2f} ± {r.std():.2f}")
...
def test(model, predictor, g, x, split_edge, evaluator, batch_size):
    ...
    for K in [20, 50, 100]:
        ...
        test_hits, test_acc, f1_score = evaluator.eval(
            {
        ...
        try: results[f"Hits@{K}"] = (train_hits, valid_hits, test_hits, test_acc, f1_score)
        except: results[f"Hits@{K}"] = (train_hits, valid_hits, test_hits)
...
def main():
    ...
    dataset = DglLinkPropPredDataset(root=args.root, name=args.dataset)
    g = dataset[0]
    ...
    if dataset.name == "ogbl-ppa":
        if args.K1 != "": g.ndata["feat"] = torch.load({}.format(args.K1)).to("cpu")
        else:
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    train_hits, valid_hits, test_hits, test_acc, test_f1 = result
                    print(key)
                    print(
                        f"Run: {run + 1:02d}, "
                        f"Epoch: {epoch:02d}, "
                        f"Loss: {loss:.4f}, "
                        f"Train: {train_hits:.2f}%, "
                        f"Valid Hits: {valid_hits:.2f}%, "
                        f"Test Hits: {test_hits:.2f}%, "
