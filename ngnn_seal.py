    ...
    def __init__(
        ...
        if "feat" in self.graph.ndata:
            if K1 != None:
                ...
                print(len(self.graph.ndata["feat"]))
                print(len(self.graph.ndata["feat"][0]))
                self.graph.ndata["feat"] = torch.load("{}".format(K1))
                print(self.graph.ndata["feat"])
                print(len(self.graph.ndata["feat"]))
                print(len(self.graph.ndata["feat"][0]))
...
@torch.no_grad()
def test(dataloader, hits_K=["hits@20","hits@50","hits@100"]):
    model.eval()
    ...
    if dataset.eval_metric.startswith("hits@"):
        ...
        results1 = evaluate_acc(pos_y_pred, neg_y_pred)
        results2 = evaluate_f1(pos_y_pred, neg_y_pred)
...
def evaluate_hits(y_pred_pos, y_pred_neg, hits_K):
    results = {}
    #hits_K = map(lambda x: (int(x.split("@")[1]) if isinstance(x, str) else x), hits_K)
    hits_K = [20,50,100]
    for K in hits_K:
        evaluator.K = K
        hits = evaluator.eval(
...
def evaluate_acc(y_pred_pos, y_pred_neg):
    results = {}
    acc = evaluator.eval(
        {
            "y_pred_pos": y_pred_pos,
...
def evaluate_f1(y_pred_pos, y_pred_neg):
    results = {}
...
if __name__ == "__main__":
    ...
    if dataset.eval_metric.startswith("hits@"):
        ...
        loggers["acc"] = Logger(args.runs, args)
        loggers["f1_score"] = Logger(args.runs, args)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                results, results1, results2 = test(val_loader, loggers.keys())
                ...
                for key, valid_res in results1.items():
                    loggers[key].add_result(run, valid_res)
                    to_print = (
                        f"Run: {run + 1:02d}, " \
                        f"Epoch: {epoch:02d}, " \
                        f"Loss: {loss:.4f}, " \
                        f"Valid ACC ({args.val_percent}%) [{key}]: {valid_res:.4f}"
                    )
                    print_log(key, to_print)
                for key, valid_res in results2.items():
                    loggers[key].add_result(run, valid_res)
                    to_print = (
                        f"Run: {run + 1:02d}, " \
                        f"Epoch: {epoch:02d}, " \
                        f"Loss: {loss:.4f}, " \
                        f"Valid F1 ({args.val_percent}%) [{key}]: {valid_res:.4f}"
                    )
                    print_log(key, to_print)
        ...
        for eval_metric in loggers.keys():
            ...
            for _idx, epoch in enumerate(idx_to_test):
                ...
                if epoch not in tested:
                    ...
                    tested[epoch] = (
                        test(final_val_loader, dataset.eval_metric)[0][dataset.eval_metric],
                        test(final_val_loader, dataset.eval_metric)[1]["acc"],
                        test(final_val_loader, dataset.eval_metric)[2]["f1_score"],
                        test(test_loader, dataset.eval_metric)[0][dataset.eval_metric],
                        test(test_loader, dataset.eval_metric)[1]["acc"],
                        test(test_loader, dataset.eval_metric)[2]["f1_score"])

                val_res, val_acc, val_f1, test_res, test_acc, test_f1 = tested[epoch]
