...
class Logger(object):
    ...
    def add_result(self, run, result):
        assert len(result) == 3 or len(result) > 3
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            if len(result[0]) > 3:
                print(f"  Highest Accuracy: {result[:, 3].max():.2f}")
                print(f"  Final Accuracy: {result[argmax, 3]:.2f}")
            ...
            for r in result:
                ...
                if len(self.results) == 3: best_results.append((train1, valid, train2, test))
                else:
                    acc = r[r[:, 1].argmax(), 3].item()
                    f1 = r[r[:, 1].argmax(), 4].item()
                    best_results.append((train1, valid, train2, test, acc, f1))
...
def test(model, predictor, g, x, split_edge, evaluator, batch_size):
    ...
    for K in [20, 50, 100]:
        ...
        try: results[f"Hits@{K}"] = (train_hits, valid_hits, test_hits[f"hits@{K}"], test_hits["acc"], test_hits["f1_score"])
        except: results[f"Hits@{K}"] = (train_hits, valid_hits, test_hits[f"hits@{K}"])
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    print(key)
                    try: train_hits, valid_hits, test_hits, test_acc, test_f1 = result
                    except: train_hits, valid_hits, test_hits = result
                    print(
                            f"Run: {run + 1:02d}, "
                            f"Epoch: {epoch:02d}, "
                            f"Loss: {loss:.4f}, "
                            f"Train hits: {train_hits:.2f}%, "
