...
from dgl.nn import GraphConv, SortPooling, SAGEConv
...
class NGNN_SAGEConv(torch.nn.Module):
    def __init__(
        ...
        self.conv = SAGEConv(input_channels, hidden_channels, "mean")
        self.fc = Linear(hidden_channels, hidden_channels)
        self.fc2 = Linear(hidden_channels, output_channels)
        self.num_layers = num_layers
...
class DGCNN(torch.nn.Module):
    def __init__(
        ...
        if GNN == "GCN":
            self.GNN = GraphConv
            self.NGNN = NGNN_GCNConv
        elif GNN == "SAGE":
            self.GNN = SAGEConv
            self.NGNN = NGNN_SAGEConv
        ...
        if ngnn_type in ["input", "all"]:
            ...
            if GNN == "SAGE": self.convs.append(self.GNN(initial_channels, hidden_channels, "mean"))
            else: self.convs.append(self.GNN(initial_channels, hidden_channels))
        ...
        if ngnn_type in ["hidden", "all"]:
            ...
            for _ in range(0, num_layers - 1):
                if GNN == "SAGE": self.convs.append(self.GNN(hidden_channels, hidden_channels, "mean"))
                else: self.convs.append(self.GNN(hidden_channels, hidden_channels))

        if ngnn_type in ["output", "all"]:
            self.convs.append(
                self.NGNN(hidden_channels, hidden_channels, 1, self.num_ngnn_layers)
            )
        else:
            if GNN == "SAGE": self.convs.append(self.GNN(hidden_channels, 1, "mean"))
            else: self.convs.append(self.GNN(hidden_channels, 1))
